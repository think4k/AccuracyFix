			...
			if (Player->IsAlive())
			{
				if (fNoMonsters == dont_ignore_monsters)
				{
					if (Player->m_pActiveItem)
					{
						...
						{
							UTIL_ClientPrintAll(HUD_PRINTTALK, "m_pActiveItem PRIMARY_WEAPON_SLOT & PISTOL_SLOT\n");
							...
							{
								UTIL_ClientPrintAll(HUD_PRINTTALK, "GetUserAiming\n");
								if (Player->pev->flags & FL_ONGROUND)
								{
									UTIL_ClientPrintAll(HUD_PRINTTALK, "FL_ONGROUND\n");
